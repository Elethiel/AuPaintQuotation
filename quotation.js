...
app.use(function(req, res, next) {
    ...
    owner.isOwnerExist(req.db, function(err, row) {
        if (err) next(err);
        else {
            if (row && row.msg === "ok") req.session.ownerId = row.ownerid;
            owner.findById(req.db, { ownerId: req.session.ownerId }, function(err, ownerObj) {
                if (err) next(err);
                else {
    ...
    if((!req.param.session || !req.param.session.username) && req.url !== "/login" && req.url !== "/in")  {
        req.param.loc = "";
